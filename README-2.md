...
> (`mediapipe.tasks.python.vision.HandLandmarker`), compatible con
> `mediapipe==0.10.35` en Python 3.10+ (macOS/Windows/Linux). La API legacy
> `mp.solutions.hands` ya no existe en estas versiones. En el primer
> arranque, `hand_tracking.py` descarga automaticamente el modelo
> `hand_landmarker.task` (unos pocos MB) desde los servidores de Google y lo
> guarda junto al proyecto; arranques posteriores no vuelven a descargarlo.
> Se requiere conexion a internet la primera vez.
